...
contract('Crew', (accounts) => {
  let token;

  beforeEach('bootstrap test suite', async () => {
    token = await Crew.new();
    await token.initialize('Influence Crew', 'INFCRW');
  });
  ...
  it('should return the correct totalSupply', async () => {
    ...
    assert.equal((await token.totalSupply.call()).toNumber(), 1);
  });

  it('should not allow a non-manager to mint a crew member', async () => {
    await truffleAssert.reverts(token.mint(accounts[1], 1, { from: accounts[1] }));
  });

  it('should allow a manager to mint a new crew member', async () => {
    let result, receipt;

    receipt = await token.mint(accounts[1], 1, { from: accounts[0] });
  ...
  it('should not allow transfers when paused', async () => {
    ...
    await truffleAssert.reverts(token.mint(accounts[1], 1, { from: accounts[0] }));
  });
  ...
  it('should not burn a non-minted token', async () => {
    await truffleAssert.reverts(token.burn(20));
  });
});
