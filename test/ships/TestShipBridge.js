...
contract('ShipBridge', (accounts) => {
  ...
  before('bootstrap test suite', async () => {
    ...
    l1TokenContract = await Ship.new();
    await l1TokenContract.initialize('Influence Ship', 'INFSHP');

    bridgeContract = await ShipBridge.new()
  ...
  describe('bridgeToStarknet', function () {
    it('should burn the token if owned', async function () {
      ...
      await truffleAssert.fails(l1TokenContract.ownerOf(1), truffleAssert.ErrorType.REVERT, 'ERC721: invalid token ID');
    });
  ...
  describe('bridgeFromStarknet', function () {
    ...
    it('should revert if the consumeMessageFromL2 fails/reverts', async function() {
      const bridge = await ShipBridge.new()
