...
contract('CrewToken', (accounts) => {
  ...
  it('should allow removing a manager', async () => {
    ...
    await token.removeManager(accounts[0]);
    await truffleAssert.reverts(token.mint(accounts[1], { from: accounts[0] }));
  });

  it('should not allow a non-manager to mint a crew member', async () => {
    await truffleAssert.reverts(token.mint(accounts[1], { from: accounts[0] }));
  });

  it('should allow setting a manager and minting a new crew member', async () => {
    let result, receipt;

    await token.addManager(accounts[0]);
    receipt = await token.mint(accounts[1], { from: accounts[0] });
    result = await token.ownerOf(1);
    assert.equal(result, accounts[1]);
    truffleAssert.eventEmitted(receipt, 'Transfer', (e) => {
      return e.from === ZERO_ADDRESS && e.to === accounts[1] && e.tokenId.toNumber() === 1;
    });
  });

  it('should increment ids of newly minted crew members', async () => {
    await token.addManager(accounts[0]);
    await token.mint(accounts[1], { from: accounts[0] });
    const receipt = await token.mint(accounts[1], { from: accounts[0] });
    truffleAssert.eventEmitted(receipt, 'Transfer', (e) => {
      return e.from === ZERO_ADDRESS && e.to === accounts[1] && e.tokenId.toNumber() === 2;
    });
  });
  ...
  it('should not allow transfers when paused', async () => {
    ...
    await token.mint(accounts[1], { from: accounts[0] });
    await token.pause();
    await truffleAssert.reverts(token.burn(1, { from: accounts[0] }));
    await truffleAssert.reverts(token.mint(accounts[1], { from: accounts[0] }));
  });
  ...
  it('shoud allow owner to transfer token', async () => {
    await token.addManager(accounts[0]);
    await token.mint(accounts[0], { from: accounts[0] });
  ...
  it('should continue incrementing token ids after burn', async () => {
    await token.addManager(accounts[0]);
    await token.mint(accounts[0], { from: accounts[0] });
    await token.burn(1);
    const receipt = await token.mint(accounts[0], { from: accounts[0] });
    truffleAssert.eventEmitted(receipt, 'Transfer', (e) => {
      return e.from === ZERO_ADDRESS && e.to === accounts[0] && e.tokenId.toNumber() === 2;
    });
  });
});
