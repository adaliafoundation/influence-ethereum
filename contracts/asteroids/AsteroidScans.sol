...
contract AsteroidScans is Pausable, Ownable {
  ...
  function setScanOrder(uint _asteroidId, uint _scanOrder) external onlyManagers {
    scanInfo[_asteroidId] = _scanOrder;
  }

  function setScanCount(uint _count) external onlyManagers {
    scanOrderCount = _count;
  }
   ...
   * @param _scanOrders An array of scan orders corresponding to _asteroidIds
   */
  function setInitialBonuses(
    uint[] calldata _asteroidIds,
    uint[] calldata _bonuses,
    uint[] calldata _scanOrders
  ) external onlyManagers {
    require(scanOrderCount == 0);
    require(_asteroidIds.length == _bonuses.length);
    require(_asteroidIds.length == _scanOrders.length);

    for (uint i = 0; i < _asteroidIds.length; i++) {
      scanInfo[_asteroidIds[i]] = _scanOrders[i];
