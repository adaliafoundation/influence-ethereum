...
contract CrewFeatures is Ownable {
  ...
  /**
   * @dev Seeds initial crewmates with collections and modifiers (for testnets)
   */
  function setInitialTokens(
    uint[] calldata _crewIds,
    uint[] calldata _collIds,
    uint[] calldata _mods
  ) external onlyManagers {
    require(_crewIds.length == _collIds.length);
    require(_crewIds.length == _mods.length);

    for (uint i = 0; i < _crewIds.length; i++) {
      _crewCollection[_crewIds[i]] = _collIds[i];
      _crewModifiers[_crewIds[i]] = _mods[i];
    }
  }
